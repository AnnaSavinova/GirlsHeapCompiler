...
void CIRTranslator::Visit( const CNewInt * newInt )
{
    ...
    IIRExp* realStart = new CIRBinOp( PLUS, mallocCall, new CIRConst( CFrame::GetWordSize() ) ); // память, сдвинутая на байт (т.к. в нулевую ячейку мы пишем длину массива)
    
    CIRCall* memsetCall = new CIRCall( memsetName, new CIRExpList( realStart, new CIRExpList( new CIRConst( 0 ), new CIRExpList( realSize, nullptr ) ) ) ); // заполняем нулями память с 1ой ячейки
    
    IIRStm* sizeSave = new CIRMove( mallocCall, arrayLength ); // пишем в нулевую ячейку длину массива
